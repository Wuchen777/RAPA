...
for module_name in list(sys.modules):
    if (
        module_name == "prismatic"
        or module_name.startswith("prismatic.")
        or module_name == "experiments"
        or module_name.startswith("experiments.")
    ):
...
def prepare_observation(
    ...
    if patch_attack:
        ...
        perturbed_wrist_img = wrist_img
        perturbed__primary_img = img
...
def run_episode(
    ...
    if cfg.patch_attack:
        ...
        top = left = top_wrist = left_wrist = 0
    try:
        while t < max_steps + cfg.num_steps_wait:
            ...
            observation, img = prepare_observation(obs, resize_size, perturbation_primary, perturbation_wrist, top,
                                                   left, top_wrist, left_wrist, patch_attack=cfg.patch_attack)
            if save_rollouts:
...
def eval_libero(cfg: GenerateConfig) -> float:
    ...
    num_tasks = task_suite.n_tasks if cfg.max_tasks <= 0 else min(cfg.max_tasks, task_suite.n_tasks)
