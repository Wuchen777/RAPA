...
class Args:
    ...
    checkpoint_path: str = os.environ.get("PI0_CHECKPOINT_PATH", "checkpoints/pi0_fast_libero")
    policy_config_name: str = os.environ.get("PI0_POLICY_CONFIG_NAME", "pi0_fast_libero")
    resize_size: int = 224
    replan_steps: int = 5
    zero_primary_camera: bool = False
    patch_after_resize: bool = False
    ...
    max_tasks: int = 0  # 0 means all tasks; use 1 for smoke tests
    eval_max_steps: int = 0  # 0 keeps the suite default horizon; >0 caps policy steps.
...
def eval_libero(args: Args) -> None:
    ...
    num_tasks_in_suite = task_suite.n_tasks if args.max_tasks <= 0 else min(args.max_tasks, task_suite.n_tasks)
    ...
    if args.eval_max_steps > 0:
        max_steps = min(max_steps, args.eval_max_steps)

    checkpoint_dir = download.maybe_download(args.checkpoint_path)
    train_config = config.get_config(args.policy_config_name)
    print(f"Pi0 policy config: {args.policy_config_name}")
    print(f"Pi0 checkpoint path: {checkpoint_dir}")
    log_file.write(f"Pi0 policy config: {args.policy_config_name}\n")
    log_file.write(f"Pi0 checkpoint path: {checkpoint_dir}\n")
    client = policy_config.create_trained_policy(train_config, checkpoint_dir)
    ...
    if args.patch_attack:
        ...
        perturbation_primary = None
        perturbation_wrist = None
    ...
    for task_id in tqdm.tqdm(range(num_tasks_in_suite)):
        ...
        for episode_idx in tqdm.tqdm(range(args.num_trials_per_task)):
            ...
            if args.patch_attack:
                ...
                top_primary = left_primary = top_wrist = left_wrist = 0
            ...
            while t < max_steps + args.num_steps_wait:
                try:
                    ...
                    if args.patch_attack and not args.patch_after_resize:
                    ...
                    if args.patch_attack and args.patch_after_resize:
                        img = apply_perturbation_to_raw_images([img], perturbation_primary, (top_primary, left_primary))[0]
                        wrist_img = apply_perturbation_to_raw_images([wrist_img], perturbation_wrist, (top_wrist, left_wrist))[0]

                    if args.zero_primary_camera:
    ...
    log_file.write(f"Overall success rate: {final_success_rate:.4f} ({final_success_rate * 100:.1f}%)\n")
    log_file.close()
