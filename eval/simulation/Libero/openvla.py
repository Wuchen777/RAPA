...
for p in [OPENVLA_ROOT, PROJECT_ROOT]:
    if p in sys.path:
        sys.path.remove(p)
    sys.path.insert(0, p)
...
class GenerateConfig:
    ...
    attn_implementation: str = "sdpa"                # Use "flash_attention_2" only when flash-attn is installed
...
def eval_libero(cfg: GenerateConfig) -> None:
    ...
    num_tasks_in_suite = task_suite.n_tasks if cfg.max_tasks <= 0 else min(cfg.max_tasks, task_suite.n_tasks)
