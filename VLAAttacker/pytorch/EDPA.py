...
class EDPA:
    def __init__(self, cfg, device_id = None):
        ...
        self.image_transform, self.get_img_embedding, self.get_lang_embedding = self._load_backend(cfg)

    def _load_backend(self, cfg):
        backend = "openvla_oft" if "oft" in str(getattr(cfg, "vla_path", "")).lower() else "openvla"
        backend = getattr(cfg, "embedding_backend", backend)
        ...
        return image_transform, get_img_embedding, get_lang_embedding
    ...
    def preprocess_tensor_images(self, images, processor=None):
        return torch.stack([
            self.image_transform(img, processor) for img in images
        ])
    ...
    def generate_one_step(
        ...
        if images_embed is None:
            ...
            images_embed = self.get_img_embedding(vla, self.preprocess_tensor_images(images, processor.image_processor))
        images_embed = images_embed.detach()
        if lang_embed is None:
            lang_embed = self.get_lang_embedding(vla, instructions)
        lang_embed = lang_embed.detach()
        ...
        for _ in range(self.iterations):
            ...
            adv_embed = self.get_img_embedding(vla, self.preprocess_tensor_images(adv_images, processor.image_processor))
    ...
    def generate(self, vla, dataloader, processor, action_tokenizer=None):
        ...
        generator = torch.Generator().manual_seed(int(getattr(self.cfg, "seed", 0)))
        perturbation = torch.zeros((3, patch_size, patch_size), dtype=torch.float32).uniform_(0, 1, generator=generator)
