...
def get_lang_embedding(vla, language_tokens):
    cache_key = "_jit_embed_fn_fast" if "FAST" in type(vla).__name__ else "_jit_embed_fn_pi0"
    if not hasattr(get_lang_embedding, cache_key):
        if "FAST" in type(vla).__name__:
            embed_fn = jax.jit(lambda x: vla.PaliGemma.llm(x, embed_only=True))
        else:
            embed_fn = jax.jit(lambda x: vla.PaliGemma.llm(x, method="embed"))
        setattr(get_lang_embedding, cache_key, embed_fn)
    embed_fn = getattr(get_lang_embedding, cache_key)
